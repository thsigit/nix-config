# modules/network/ap/hostapd.nix
#
# Access-point radio via hostapd. Part of the services.ap bundle (see ap/default.nix).
# Everything here is inert unless services.ap.enable = true.
...
in
{
  config = lib.mkIf config.services.ap.enable {
    ...
    # The generated network-addresses-${ap}.service is WantedBy the wifi device
    # unit. When iwlwifi is still initializing at boot (radio soft-blocked), the
    # device trigger can be missed, leaving the AP interface without 192.168.4.1
    # and crash-looping dnsmasq ("Cannot assign requested address"). Pull the
    # address unit in at multi-user.target too; After/BindsTo on the device unit
    # still make it wait for the interface before assigning the address.
    systemd.services."network-addresses-${ap}".wantedBy = [ "multi-user.target" ];
    ...
    # took DHCP/DNS down too. Unblock the radio before hostapd starts.
