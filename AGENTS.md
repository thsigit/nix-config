...
- One repo, two branches in `/srv/repo/nix-lab`: branch `dev` = active development, branch `main` = public release (the deployed end product). `/srv/repo/nix-config` is an inert archive. See `WORKFLOW.md`. GitHub's nix-config is the **rolling release** (`main`).
