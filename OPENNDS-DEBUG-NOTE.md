...
- AP stack = single toggle `services.ap.enable`; files in `modules/network/ap/`
...
- `/srv/repo/nix-config/modules/network/ap/*.nix` — AP config
...
- Post-refactor invariant: `modules/ai` and `modules/network` must not mix AI vs Internet
